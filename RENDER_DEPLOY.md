...
> `DATA_ROOT` / `STORAGE_ROOT` 默认为相对路径 `./data` 与 `./storage`，在容器内 cwd 为 `/app`，因此自动解析为 `/app/data` 与 `/app/storage`，正好命中下方挂载的持久化磁盘。如需自定义可改为绝对路径。
...
- `BGM_PATH=`（留空则使用低音量占位音；填入 `.mp3` 路径可启用真实 BGM，建议放到 `/app/storage/assets/bgm/`）
...
|---|---|---|
| `data-disk` | `/app/data` | SQLite 数据库 `data.db`（产品库 + 历史记录） |
| `storage-disk` | `/app/storage` | 上传图片、临时素材、配音、输出视频等缓存 |

配置要点：
1. 在 Render 服务 → **Disks** 中创建两块 Disk，分别挂载到上述路径。
2. Disk 容量建议：`data-disk` 1 GB 即可；`storage-disk` 10 GB 起（视频产物较大，可按需扩容）。
3. 应用启动时（`app/main.py` 与 `app/core/database.py`）会 **自动创建** `data`、`storage` 及其子目录（`outputs/`、`uploads/`、`temp/`、`audios/`、`assets/`、`assets/bgm/`），无需手动初始化。
...
- [ ] 两块 Disk 已挂载到 `/app/data` 与 `/app/storage`
- [ ] 首次启动后检查 `/app/data/data.db` 是否自动创建
- [ ] 发起一次生成任务，确认 `/app/storage/outputs` 产出最终视频，且字幕为中文（非方块）
- [ ] 重新部署后，旧的历史记录（`/app/data/data.db`）与已生成视频仍在

## 五、已知架构说明（非阻塞）
任务状态当前保存在进程内存并由 Gunicorn 多 worker 并发处理。若后续并发量上升需要跨 worker/跨重启保留进度，可将任务调度切到真实 Celery + Redis（`requirements.txt` 已包含依赖）。当前单实例 + 持久化磁盘已可支撑上线。
